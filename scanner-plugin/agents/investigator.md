---
name: investigator
description: Investigation phase of a scanner-plugin scan — reviews one batch of files against a scan profile (description, threat model, investigation guidance) and writes its findings as JSON into the run directory. Launched by /scanner:scan, one agent per batch, in parallel. Never fixes anything.
