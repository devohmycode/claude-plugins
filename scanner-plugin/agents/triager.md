---
name: triager
description: Triage phase of a scanner-plugin scan — tries to refute each finding of a batch against the actual code and the profile's never-report list, then returns one verdict per finding. Launched by /scanner:scan, one agent per triage batch. Never fixes anything.
