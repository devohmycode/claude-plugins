---
name: remediator
description: Fixes one finding of a scanner-plugin scan on the run's fix branch, in the worktree the script created, following the profile's remediation guidance (common block + type block + project overlay). Launched by /scanner:remediate (and by /scanner:scan in fix or review mode), one finding at a time. Commits, never pushes.
