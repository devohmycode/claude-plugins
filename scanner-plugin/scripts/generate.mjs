...
for (const file of bases) {
  ...
  for (const effort of EFFORTS) {
    ...
    const frontmatter = [
      `name: ${name}-${effort}`,
      `description: scanner:${name} at ${effort} reasoning effort — same role and instructions. Launched by the scanner commands when the scan type's effort is ${effort}.`,
      `tools: ${field('tools')}`,
      `effort: ${effort}`,
    ]
...
for (const type of builtinTypes()) {
  rows[agentOptionKey(type, 'model')] = {
    type: 'string',
    title: `Model — ${type}`,
    description: `Model of the ${type} scan's agents: inherit (the session's model), haiku, sonnet, opus or fable.`,
    options: AGENT_SETTINGS.model.values,
    default: 'inherit',
  }
  rows[agentOptionKey(type, 'effort')] = {
    type: 'string',
    title: `Effort — ${type}`,
...
if (problems) {
  console.log(`\n${problems} problem(s): run node scanner-plugin/scripts/generate.mjs`)
  process.exit(1)
}
