...
`/tracker:triage priority:P1` prepares a run, then launches **one triager per issue, in
parallel**. A triager re-measures the issue against the code as it is now and says `holds`,
...
2. one **fixer** per issue, one after the other: it follows the project's conventions
...
- `triage` — `max`, `relabel`, `commentOnHolds`.
- `batch` — `base`, `prBase`, `branchPrefix`, `perBatch`, `max`, `groupBy`
  (`area`, `axis`, `none`), `setup`, `checks`, `checkTimeoutMinutes`, `draft`.
...
top-level value, `/config`, inherit.
